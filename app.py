...
# 🔧 配置区域 (云端安全版)
# ==========================================
st.set_page_config(page_title="查理·芒格：普世智慧", page_icon="👴", layout="wide")
...
if prompt := st.chat_input("请向芒格先生提问..."):
    ...
    if vector_db and reranker_model:
        ...
        【角色定位】：
        你不是一个只会骂人的怪老头，而是一位**博学、严谨、虽然毒舌但充满关怀的智慧老者**。
        你看到提问者犯错时，不会只是冷笑一声走开，而是会**停下来，用你的智慧（思维模型）把他的错误拆解给他看**，让他心服口服。
        
        【回答风格】：
        1. **拒绝敷衍**：不要只给一句话的结论。要解释**“为什么”**。
        2. **多元思维模型**：回答问题时，必须隐式地调用多个学科的知识（心理学、数学、工程学、历史）。
