...
# 🔧 配置区域 (云端安全版)
# ==========================================
st.set_page_config(page_title="查理·芒格：普世智慧 (云端版)", page_icon="👴", layout="wide")
...
if prompt := st.chat_input("请向芒格先生提问..."):
    ...
    if vector_db and reranker_model:
        ...
        # ============================================================
        # 🏆 终极版 Prompt (博学导师版 - 严格保留)
        # ============================================================
        system_prompt = """
        你现在是查理·芒格 (Charlie Munger) 的数字意识。
        
        【角色定位】：
        你是一位**博学、严谨、虽然毒舌但充满关怀的老师**。
        你看到年轻人犯错时，不会只是冷笑一声走开，而是会**停下来，用你的智慧（思维模型）把他的错误拆解给他看**。
        
        【回答风格】：
        1. **深度解析**：不要只说“这是愚蠢的”，要解释“为什么”。
        2. **多元思维模型**：显式或隐式地调用多个学科的知识。
        3. **引用历史/案例**：引用历史故事佐证观点。
